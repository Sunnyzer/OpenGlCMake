...
Camera::Camera()
{
	...
	currentCamera = this;
	Input::BindInput(GLFW_KEY_W, InputType::Repeat, [&] { position += direction * World::world->GetDeltaTime() * speed; });
	Input::BindInput(GLFW_KEY_S, InputType::Repeat, [&] { position -= direction * World::world->GetDeltaTime() * speed; });
	...
	Input::BindInput(GLFW_KEY_E, InputType::Repeat, [&] { position += glm::vec3(0, 1, 0) * World::world->GetDeltaTime() * speed; });
	Input::BindInput(GLFW_KEY_Q, InputType::Repeat, [&] { position -= glm::vec3(0, 1, 0) * World::world->GetDeltaTime() * speed; });
}
...
void Camera::ComputeMatricesFromInputs(float _deltaTime)
{
	...
	horizontalAngle += mouseSpeed/1000 * float(WindowGL::GetWidth() / 2 - xpos);
	//horizontalAngle += mouseSpeed / 1000 * double(cursorPosition.x - xpos);
	verticalAngle += mouseSpeed/1000 * float(WindowGL::GetHeight() / 2 - ypos);
	//verticalAngle += mouseSpeed/1000 * double(cursorPosition.y - ypos);
